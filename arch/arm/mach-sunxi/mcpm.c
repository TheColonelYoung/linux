...
static void sunxi_cpu_cache_disable(void)
{
	...
	pr_info("CPUidle Allwinner A83t: Cluster %d CPU %u cache disable\n", cluster, cpu);
...
static void sunxi_cluster_cache_disable(void)
{
	...
	pr_info("CPUidle Allwinner A83t: Cluster %u cache disable\n", cluster);
...
static int __init sunxi_mcpm_init(void)
{
	...
	pr_info("sunxi MCPM cpucfg_base: %ld",(long)cpucfg_base);
	of_node_put(node);
	if (!cpucfg_base) {
